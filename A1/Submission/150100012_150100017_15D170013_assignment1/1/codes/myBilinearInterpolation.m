function [modified_pic] = myBilinearInterpolation(original_pic,m,n)
	...
	for i=1:num_chan
		% Setting the dimensions of the new modified image %
		h_new = m*(h-1)+1;
		w_new = n*(w-1)+1;
		% Initialising the image with a blank image %
		modified_pic(:,:,i) = zeros(h_new,w_new);
		for x = 1:h_new
			for y = 1:w_new
				% Bilinearly Interpolating depending on the case %
				x_rem = mod(x-1,m);
				y_rem = mod(y-1,n);
				% Vertex case %
				if(x == h_new && y==w_new)
	...
	% Scaling all the values to lie between 0 and 1 %
