...
padded_pic = padarray(original_pic,[floor(h/2),floor(w/2)],0,'both');
fft_pic = fftshift(fft2(padded_pic));
...
mid_pic = unpad(real(ifft2(ifftshift(mid_fft_pic))));
modified_pic = unpad(real(ifft2(ifftshift(modified_fft_pic))));
...
% 3. Comparing across the filters, we see clearly ringing artifacts when ideal low pass filter is applied in both the cases    

%% Function to extract the required part of the resultant image
function modified_pic = unpad(original_pic)
	[h,w] = size(original_pic);
	modified_pic = original_pic((h/4)+1:3*(h/4),(w/4)+1:3*(w/4));
end
